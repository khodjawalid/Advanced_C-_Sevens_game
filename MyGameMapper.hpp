...
public:
    ...
    void runCompetition(const std::vector<std::shared_ptr<PlayerStrategy>>& strategies);
...
private:
    std::unordered_map<uint64_t, std::shared_ptr<PlayerStrategy>> strategies_;
    std::unordered_map<uint64_t, std::vector<Card>> playerHands_;
    std::unordered_map<uint64_t, std::unordered_map<uint64_t, bool>> tableLayout_;
    std::mt19937 rng_;
};
